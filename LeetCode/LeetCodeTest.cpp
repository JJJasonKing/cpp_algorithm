...
int mainln1() {
    ...
    int n;
    cin >> n;
    vector<vector<string>> tmp(n, vector<string>(2));
    vector<int> nums(n);
    for (int i = 0; i < n; ++i) {
        cin >> tmp[i][0] >> tmp[i][1];
        if (tmp[0][0] == "out") {
        ...
        // xx:yy:zz
        int hour = stoi(tmp[i][1].substr(0, 2));
        int min = stoi(tmp[i][1].substr(3, 2));
        int ss = stoi(tmp[i][1].substr(6, 2));
        nums[i] = hour * 3600 + min * 60 + ss;
    }
    long res = 0;
    for (int i = 1; i < n; ++i) {
        if (tmp[i][0] == "out" && tmp[i - 1][0] == "in" && nums[i] > nums[i - 1]) {
            res += (nums[i] - nums[i - 1]);
        }
    }
    string hour = to_string(res / 3600);
    if (hour.size() < 2) {
        hour = "0" + hour;
    }
    res %= 3600;
...
int mainl2() {
    ...
    while (q--) {
        ...
        if (type == 1) {
            int t = nums[rc][n - 1];
            for (int i = n - 1; i > 0; --i) {
                nums[rc][i] = nums[rc][i - 1];
            }
            nums[rc][0] = t;
        } else { // 列向上1
            int t = nums[0][rc];
            for (int i = 0; i < n - 1; ++i) {
                nums[i][rc] = nums[i + 1][rc];
            }
            nums[n - 1][rc] = t;
        }
    }
